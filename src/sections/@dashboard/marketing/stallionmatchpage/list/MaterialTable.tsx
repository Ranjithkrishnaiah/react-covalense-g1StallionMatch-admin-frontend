...
import { CarasoulDataItem } from "src/pages/dashboard/MarketingStallionMatchData";
...
export const MaterialTable: React.FC<{ items: CarasoulDataItem[], CaraoulSectionUuid: any, openAddEditForm: boolean, handleDrawerCloseRow: VoidFunction, 
  ...
  clickedPopover: any, setClickedPopover: React.Dispatch<React.SetStateAction<any>>}> = (
    { items, CaraoulSectionUuid, openAddEditForm, handleDrawerCloseRow, apiStatus, setApiStatus, 
      apiStatusMsg, setApiStatusMsg, marketingModuleAccess, setMarketingModuleAccess, clickedPopover, setClickedPopover }) => {
  // cache the items provided via props in state for purposes of this demo
  
  const [localItems, setLocalItems] = useState<Array<CarasoulDataItem>>(items);
  ...
  const handleDragEnd = async(result: DropResult, provided?: ResponderProvided) => {
    if (!marketingModuleAccess?.marketing_update) {
      ...
      // console.log('carasoul position array', positionArr);
  ...
  items?.map((item: any, index: number)=>{
    ...
    // setApiPositionedUuids([item.position] = item.id);
  });
      ...
      <Table sx={{borderCollapse: 'separate', borderSpacing : '0 4px', background: '#FAF8F7'}}>
        <TableHead>
            <TableRow>    
                ...
                <TableCell align="left">Button Text</TableCell>
        ...
        <DragDropContext onDragEnd={handleDragEnd}>
          <Droppable droppableId="droppable" direction="vertical">
            {(droppableProvided: DroppableProvided) => (
              ...
              >
                {localItems && localItems?.map((item: CarasoulDataItem, index: number) => (
                  <Draggable
                    key={item?.id}
                    draggableId={item?.id}
                    index={item?.position}
                    ...
                    {(
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell align="left"><Typography variant='h5' sx={{textTransform:'capitalize'}}>{item?.buttonText}</Typography></TableCell>
                          <TableCell align="left"><Typography variant='h5'>{item?.orientation}</Typography></TableCell>
