...
import { useHorseDetailsByIdQuery, useAddHorseMutation, useEditHorseMutation } from 'src/redux/splitEndpoints/horseSplit';
...
export default function PedigreeNewlyAddedEditModal(props: any) {
  ...
  const {
    ...
    progenyId,
    selectedIndex,
    apiStatus, setApiStatus, apiStatusMsg, setApiStatusMsg, getHorseDetails, pedigreeHorseId, 
    setPedigreeHorseId, pedigreeHorseTag, isPedigreeHorseSearch, setIsPedigreeHorseSearch,
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      totalPrizeMoneyEarned: (isEdit && currentPedigreeHorse?.totalPrizeMoneyEarned) ? currentPedigreeHorse?.totalPrizeMoneyEarned : "",     
  ...
  const onSubmit = async (data: FormValuesProps) => {  
    ...
    const finalData = { ...data, yob: (!data?.yob) ? null : Number(yob), sex: sex, gelding: (gelding === 'true') ? true : false, dob: (!dateValue) ? null : dateValue, totalPrizeMoneyEarned: (Number(data.totalPrizeMoneyEarned) === 0) ? null : Number(data.totalPrizeMoneyEarned), currencyId: (!state?.currencyId) ? null : Number(state?.currencyId) }
    ...
    try { 
      ...
      if(res?.data){
        ...
        setPedigreeHorseId({
          pedigreeId: isEdit ? rowId : res?.data?.horseUuid, 
          tag: pedigreeHorseTag
        })  
        reloadPedigreeAfterSaveOrUpdate();        
