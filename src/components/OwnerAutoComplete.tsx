...
import { useRunnersOwnerQuery } from 'src/redux/splitEndpoints/runnerDetailsSplit';
import CloseRoundedIcon from '@mui/icons-material/CloseRounded';
import { TextField } from '@mui/material';

export default function OwnerAutoComplete(props: any) {
  ...
  const { data, error, isFetching, isLoading, isSuccess } = useRunnersOwnerQuery(farmFilterData, { skip: (!isFarm) });
  ...
  return (
    <CustomAutocomplete
      ...
      renderInput={(params: any) => 
        <TextField {...params} error={((props?.isError !== undefined && props?.isError?.length !== 0) && selectedFarm.displayName?.length === 0) ? true : false} placeholder="Owner(s)" />}
    />
