...
const DottedWorldStallionMap = (props: StallionDashboardProps) => {
  const [isTooltipVisible, setIsTooltipVisible] = useState(false);
  const [displayedColiving, setDisplayedColiving] = useState<any>(null);
  const [tooltipCoords, setTooltipCoords] = useState({ top: 0, left: 0 });
  const { fromDate, toDate, totalStallions } = props;
  // Stallion world map api call
  const { data: worldmapAPIData, isFetching: isWRFetching, isLoading: isWRLoading, isSuccess: isWRSuccess  } = useStallionsDashboardWorldReachQuery({'fromDate': fromDate, 'toDate': toDate});
    ...
    (isWRSuccess && <div className="dottedMapmain">
      ...
      >
        ...
        {worldmapAPIData.map((coliving: any) => {
          ...
          let pinRadius = (coliving?.StallionCount / totalStallions) * 100;
      ...
      >
        ...
        <Stack className='Tooltipmapbox'>
          {displayedColiving?.StallionCount > 0 && <b>{displayedColiving?.StallionCount} Stallions</b>}
          {displayedColiving?.PromotionCount > 0 && <b>{displayedColiving?.PromotionCount} Promoted</b>}
        </Stack>
        {/* End of World map tooltip data section */}
      </Tooltip>
...
export default DottedWorldStallionMap;