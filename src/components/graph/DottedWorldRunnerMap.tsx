...
const DottedWorldRunnerMap = (props: RaceDashboardProps) => {
  const [isTooltipVisible, setIsTooltipVisible] = useState(false);
  const [displayedColiving, setDisplayedColiving] = useState<any>(null);
  const [tooltipCoords, setTooltipCoords] = useState({ top: 0, left: 0 });
  const { fromDate, toDate, totalRunners ,countryId} = props;
  const { data: worldmapAPIData, isFetching: isWRFetching, isLoading: isWRLoading, isSuccess: isWRSuccess  } = useRunnerDashboardWorldReachQuery({'fromDate': fromDate, 'toDate': toDate,'countryId':countryId?.join()},{ skip: (!(countryId?.length > 0)),refetchOnMountOrArgChange: true });
    ...
    (isWRSuccess && <div className="dottedMapmain">
      ...
      >
        ...
        {worldmapAPIData.map((coliving: any) => {
          ...
          let pinRadius = (coliving?.runnerCount / totalRunners) * 100;
      ...
      >
        <Stack className='Tooltipmapbox'>
          ...
          <p>{displayedColiving?.stateName}, {displayedColiving?.countryName} [{(displayedColiving?.runnerCount) ? intToString(displayedColiving?.runnerCount): 0}]</p>
        </Stack>
      </Tooltip>
    </div>
    )}
...
export default DottedWorldRunnerMap;