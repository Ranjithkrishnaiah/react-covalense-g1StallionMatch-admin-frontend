...
export default function CustomFilterRangePicker(props: any) {
  const outlinedInputRef = React.useRef<HTMLInputElement>(null); // Create a ref for the OutlinedInput
  ...
  // const outlinedInputRef = React.createRef(); // Create a ref for the OutlinedInput
  ...
  // Updated values whenever the convertedDateRangeValue is updated
  React.useEffect(() => {
  ...
  return (
    <StyledEngineProvider injectFirst>
      <Box className="customDate" onKeyDown={props?.handleSelectKeyDown} tabIndex={0}>
        <FormControl className="datepicker" variant="filled">
          <OutlinedInput
            ...
            inputRef={outlinedInputRef} // Assign the ref to the OutlinedInput
            onKeyDown={(e) => {
              if (e.key === 'Backspace') {
