...
function CustomDropzoneProfileImage(dataProps: any) {
  ...
  const [fileSelected, setFileSelected] = useState<any>(props?.fileDetails === null ? false : true);
  ...
  if (!fileSelected) {
    thumbs = files.map((file: any) => {
      ...
      if (!fileError.message) {
        return (
          <Box className={`draganddrop hero-pic ${dataProps?.isStallion ? 'stallionDropzon' : ''}`} key={file.name}>
  ...
  if (fileSelected) {
    thumbs = (
      <Box className={`draganddrop hero-pic ${dataProps?.isStallion ? 'stallionDropzon' : ''}`} key={props.fileDetails}>
        <i className="icon-Incorrect" onClick={(e) => removeFileExisting()} />
        <Box className="draganddropImg">
          {dataProps?.isStallion ?
            <img src={`${props.fileDetails}?w=200&h=200&mask=ellipse&ar=1:1&fit=crop`} alt="stallion" /> :
            <img src={`${props.fileDetails}?w=200&h=200`} alt="farm" />
          }
        </Box>
      </Box>
    );
  ...
  if (fileSelected && props.fileDetails?.fileName) {
    ...
    imageFile = imageFormats.includes(type) ? (
      <img src={props.fileDetails?.mediaUrl} alt={props.fileDetails?.fileName} />
    ) : (
      ''
    );
    ...
    thumbs = (
      <Box className={`draganddrop hero-pic ${dataProps?.isStallion ? 'stallionDropzon' : ''}`} key={props.fileDetails?.fileName}>
        <i className="icon-Incorrect" onClick={(e) => removeFileExisting()} />
        <Box className="draganddropImg">
          {imageFile} {videoFile}
        </Box>
  ...
  const removeFileExisting = () => {
    ...
    setDisplayed(true);
    setIsDeleted(true);
    props.setFileUpload({ isDeleted: true, isNew: false });
    setFileError(false);
  };
    ...
    <>
      <StyledEngineProvider injectFirst>
        ...
        {!fileSelected && isDisplayed ? (
          <Box className={`draganddrop ${dataProps?.isStallion ? 'stallionDropzon' : ''}`} {...getRootProps({ style })} sx={{ height: '136px' }}>
            <input {...getInputProps()} />
...
export default CustomDropzoneProfileImage;
