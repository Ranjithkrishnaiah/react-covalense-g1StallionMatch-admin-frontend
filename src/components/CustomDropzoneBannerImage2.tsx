...
function CustomDropzoneBannerImage2(dataProps: any) {
  const props: any = dataProps.data;
  const tempFileName: any = uuid();
  // states
  const [isDisplayed, setDisplayed] = useState<any>(true);
  const [files, setFiles] = useState([]);
  const [fileSelected, setFileSelected] = useState(props.banner2ImagesDetails);
  ...
  const onDrop = useCallback((acceptedFiles) => {
    ...
    setFiles(
      acceptedFiles.map((file: any) => {
        ...
        props.setBanner2ImageFileUpload(file);
  ...
  if (fileSelected) {
    thumbs = (
      <Box className="draganddrop hero-pic" key={props.banner2ImagesDetails}>
        <i className="icon-Incorrect" onClick={(e) => removeFileExisting()} />
        <Box className="draganddropImg">
          <img src={`${props.banner2ImagesDetails}?w=${dataProps?.size?.w}&h=${dataProps?.size?.h}`} alt="farm" />
        </Box>
      </Box>
    );
  }
  // if fileSelected and fileName present
  if (fileSelected && props.banner2ImagesDetails?.fileName) {
    let imageFile, videoFile;
    const type = props.banner2ImagesDetails?.fileName.split('.')[1];
    imageFile = imageFormats.includes(type) ? (
      <img src={`{props.banner2ImagesDetails?.mediaUrl}?w=${dataProps?.size?.w}&h=${dataProps?.size?.h}`} alt={props.banner2ImagesDetails?.fileName} />
    ) : (
      ''
    );
    videoFile = videoFormats.includes(type) ? (
      <video src={props.banner2ImagesDetails?.mediaUrl} width="350" height="350" controls />
    ) : (
      ''
    );
    thumbs = (
      <Box className="draganddrop hero-pic" key={props.banner2ImagesDetails?.fileName}>
        <i className="icon-Incorrect" onClick={(e) => removeFileExisting()} />
        <Box className="draganddropImg">
          {imageFile} {videoFile}
        </Box>
  ...
  const removeFile = (file: any) => {
    setDisplayed(true);
    props.setBanner2ImageFileUpload({ isDeleted: file.tempFileName, isNew: true });
  ...
  const removeFileExisting = () => {
    ...
    setDisplayed(true);
    props.setBanner2ImageFileUpload({ isDeleted: true, isNew: false });
    setFileError(false);
  };
...
export default CustomDropzoneBannerImage2;
