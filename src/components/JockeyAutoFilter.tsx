...
import { useRunnersJockeyQuery } from 'src/redux/splitEndpoints/runnerDetailsSplit';
import CloseRoundedIcon from '@mui/icons-material/CloseRounded';
import { TextField } from '@mui/material';

export default function JockeyAutoFilter(props: any) {
  ...
  const { data, error, isFetching, isLoading, isSuccess } = useRunnersJockeyQuery(farmFilterData, { skip: (!isFarm) });
  ...
  return (
    <CustomAutocomplete
      ...
      renderInput={(params: any) => <TextField {...params} error={((props?.isError !== undefined && props?.isError?.length !== 0) && selectedFarm.displayName?.length === 0) ? true : false} placeholder="Jockey" />}
    />
